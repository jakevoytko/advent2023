...
q = [(-board[0][0], 0, 0, Direction.DOWN, 0, None), (-board[0][0], 0, 0, Direction.RIGHT, 0, None)]
minMap = {}
...
while len(q) > 0:
  ...
  if row == len(board) - 1 and col == len(board[0]) - 1 and pathLength >= 4:
    print(cost)
    break
  ...
  for (rowOffset, colOffset, newDirection, continuation) in directionMap[direction]:
    ...
    if continuation and pathLength + 1 > 10 or not continuation and pathLength < 4:
      continue
    if next[1] >= 0 and next[1] < len(board) and next[2] >= 0 and next[2] < len(board[0]):
